 ...
 * necessary.
 *
 * @param {string} relativePath - A path relative to Zorin Connect's resource path
...
function installService() {
    ...
    const manifestFile = 'org.gnome.shell.extensions.zorin_connect.json';
    ...
    if (Config.IS_USER) {
        ...
        if (!_installResource(dbusDir, dbusFile, `${dbusFile}.in`))
            throw Error('Zorin Connect: Failed to install DBus Service');
        ...
        // File Manager Extensions
        const target = `${Config.PACKAGE_DATADIR}/nautilus-zorin-connect.py`;
