...
var RadioButton = GObject.registerClass({
    GTypeName: 'ZorinConnectShellRadioButton'
    ...
    _init(params) {
        ...
        super._init({
            style_class: 'zorin-connect-radio-button',
            vertical: false
        });
...
var Dialog = class Dialog extends ModalDialog.ModalDialog {
    ...
    constructor() {
        super({styleClass: 'zorin-connect-dnd-dialog'});
        ...
        let radioList = new St.BoxLayout({
            style_class: 'zorin-connect-radio-list',
            vertical: true
        });
        ...
        let timer = new St.BoxLayout({
            vertical: false,
            x_expand: true,
            style_class: 'zorin-connect-dnd-timer'
        });
    ...
    _done() {
        ...
        zorin_connect.settings.set_int('donotdisturb', time);
        this.close();
    }
    ...
    _getDurationLabel() {
        if (this._time >= 60 * 60) {
            ...
            return zorin_connect.ngettext('%d hour', '%d hours', hours).format(hours);
        } else {
            // TRANSLATORS: Time duration in minutes (eg. 15 minutes)
            return zorin_connect.ngettext('%d minute', '%d minutes', (this._time / 60)).format(this._time / 60);
        }
    }
...
var MenuItem = class MenuItem extends PopupMenu.PopupSwitchMenuItem {
    ...
    constructor() {
        ...
        this.actor.connect('notify::mapped', () => {
            ...
            this.setToggleState(zorin_connect.settings.get_int('donotdisturb') > now);
        });
