...
Zorin Connect Version: ${zorin_connect.metadata.version}
Zorin Connect Install: ${(zorin_connect.is_local) ? 'user' : 'system'}
...
async function generateSupportLog(time) {
    try {
        let file = Gio.File.new_tmp('zorin-connect.XXXXXX')[0];
...
var ConnectDialog = GObject.registerClass({
    ...
    Template: 'resource:///org/gnome/Shell/Extensions/ZorinConnect/connect.ui',
    Children: [
...
const DeviceRow = GObject.registerClass({
    GTypeName: 'ZorinConnectDeviceRow',
    Properties: {
...
}, class ZorinConnectDeviceRow extends Gtk.ListBoxRow {
...
var Window = GObject.registerClass({
    GTypeName: 'ZorinConnectSettingsWindow',
    Properties: {
    ...
    Template: 'resource:///org/gnome/Shell/Extensions/ZorinConnect/settings.ui',
    Children: [
        // HeaderBar
    ...
    _init(params) {
        ...
        this.settings = new Gio.Settings({
            settings_schema: zorin_connect.gschema.lookup(
                'org.gnome.Shell.Extensions.ZorinConnect.Preferences',
                true
            ),
            path: '/org/gnome/shell/extensions/zorin-connect/preferences/'
        });

        zorin_connect.settings.bind(
            'discoverable',
            this.infobar,
            'reveal-child',
        ...
        this.headerbar.title = zorin_connect.settings.get_string('public-name');
        ...
        // Setup devices
        this._devicesChangedId = zorin_connect.settings.connect(
            'changed::devices',
    ...
    get display_mode() {
        if (zorin_connect.settings.get_boolean('show-indicators')) {
            return 'panel';
        } else {
            return 'user-menu';
        }
    ...
    set display_mode(mode) {
        zorin_connect.settings.set_boolean('show-indicators', (mode === 'panel'));
    }
        ...
        if (this._about === undefined) {
            this._about = new Gtk.AboutDialog({
                ...
                logo: GdkPixbuf.Pixbuf.new_from_resource_at_scale(
                    zorin_connect.app_path + '/icons/' + zorin_connect.app_id + '.svg',
                    128,
                    128,
                    true
                ),
                program_name: 'Zorin Connect',
                // TRANSLATORS: eg. 'Translator Name <your.email@domain.com>'
                translator_credits: _('translator-credits'),
                version: `${zorin_connect.metadata.version}`,
    ...
    _onSetServiceName(button, event) {
        if (this.service_entry.text.length) {
            ...
            zorin_connect.settings.set_string('public-name', this.service_entry.text);
        }
