...
const _debugFunc = function(message, prefix = null) {
    ...
    let script = file.replace(zorin_connect.extdatadir, '');

    GLib.log_structured('ZorinConnect', GLib.LogLevelFlags.LEVEL_MESSAGE, {
        'MESSAGE': `[${script}:${func}:${line}]: ${message}`,
        'SYSLOG_IDENTIFIER': 'org.gnome.Shell.Extensions.ZorinConnect',
        'CODE_FILE': file,
        'CODE_FUNC': func,
        'CODE_LINE': line
    });
...
window.debug = zorin_connect.settings.get_boolean('debug') ? _debugFunc : () => {};

zorin_connect.settings.connect('changed::debug', (settings) => {
...
Gio.TlsCertificate.new_for_paths = function (cert_path, key_path) {
    ...
    if (!cert_exists || !key_exists) {
        let proc = new Gio.Subprocess({
            argv: [
                zorin_connect.metadata.bin.openssl, 'req',
                '-new', '-x509', '-sha256',
                '-out', cert_path,
                '-newkey', 'rsa:4096', '-nodes',
                '-keyout', key_path,
                '-days', '3650',
                '-subj', '/O=Zorin OS/OU=Zorin Connect/CN=' + GLib.uuid_string_random()
            ],
    ...
    'fingerprint': {
        value: function() {
            if (!this.__fingerprint) {
                let proc = new Gio.Subprocess({
                    argv: [zorin_connect.metadata.bin.openssl, 'x509', '-noout', '-fingerprint', '-sha1', '-inform', 'pem'],
    ...
    'common_name': {
        get: function() {
            if (!this.__common_name) {
                let proc = new Gio.Subprocess({
                    argv: [zorin_connect.metadata.bin.openssl, 'x509', '-noout', '-subject', '-inform', 'pem'],
                    flags: Gio.SubprocessFlags.STDIN_PIPE | Gio.SubprocessFlags.STDOUT_PIPE
                });
                proc.init(null);
    ...
    'certificate_der': {
        get: function() {
            if (!this.__certificate_der) {
                let proc = new Gio.Subprocess({
                    argv: [zorin_connect.metadata.bin.openssl, 'x509', '-outform', 'der', '-inform', 'pem'],
                    flags: Gio.SubprocessFlags.STDIN_PIPE | Gio.SubprocessFlags.STDOUT_PIPE
                });
                proc.init(null);
