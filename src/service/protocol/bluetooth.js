 ...
 */
const SERVICE_RECORD = zorin_connect.get_resource(`${zorin_connect.app_id}.sdp.xml`);
...
var ChannelService = GObject.registerClass({
    GTypeName: 'ZorinConnectBluetoothChannelService',
    ...
    _register(uuid) {
        ...
        this._profile = new DBus.Interface({
            ...
            g_object_path: zorin_connect.app_path + uuid.replace(/-/gi, '')
        });
    ...
    async _init_async() {
        try {
            ...
            warning(`Zorin Connect: Bluetooth Error: ${e.message}`);
            this.destroy();
        }
    }
    ...
    async RequestDisconnection(object_path) {
        ...
        try {
            ...
            if (device && device._channel !== null) {
                debug(`Zorin Connect: Disconnecting ${device.Alias}`);
