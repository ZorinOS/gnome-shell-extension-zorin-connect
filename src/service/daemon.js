...
const Service = GObject.registerClass({
    ...
    _migrateConfiguration() {
        ...
        const deviceList = this.settings.get_strv('devices').filter(id => {
            const settingsPath = `/org/gnome/shell/extensions/zorin-connect/device/${id}/`;
            ...
            const settings = new Gio.Settings({
                settings_schema: Config.GSCHEMA.lookup(
                    'org.gnome.Shell.Extensions.ZorinConnect.Device', true),
                path: settingsPath,
            });
        ...
        notification.set_body(_('Zorin Connect has updated to support changes to the KDE Connect protocol. Some devices may need to be repaired.'));
