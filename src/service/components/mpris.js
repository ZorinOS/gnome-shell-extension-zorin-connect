...
export const Player = GObject.registerClass({
    GTypeName: 'ZorinConnectMediaPlayerInterface',
    Properties: {
...
const PlayerProxy = GObject.registerClass({
    GTypeName: 'ZorinConnectMPRISPlayer',
}, class PlayerProxy extends Player {

    /**
     * @constructs ZorinConnectMPRISPlayer
...
const Manager = GObject.registerClass({
    GTypeName: 'ZorinConnectMPRISManager',
    Signals: {
        'player-added': {
    ...
    async _loadPlayers() {
        try {
            ...
            for (let i = 0, len = names.length; i < len; i++) {
                ...
                if (!name.includes('Zorin Connect'))
     ...
     *
     * @param {string} identity - A player name
     * @returns {ZorinConnectMPRISPlayer|null} A player or %null
     */
    getPlayer(identity) {
        for (const player of this._players.values()) {
            if (player.Identity === identity)
